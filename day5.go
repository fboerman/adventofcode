...
func main() {
	...
	for pc:=0; pc < len(mem); {
		...
		instr_str = sb.String()
		fmt.Println("DEBUG: instr_str: ", instr_str)
		//slice the last two, this is the opcode
		opcode, _ := strconv.Atoi(instr_str[3:])
		fmt.Println("DEBUG: opcode: ", opcode)
		switch opcode {
		case 1:
			// addition
			fmt.Println("DEBUG: full instruction: ", mem[pc:pc+4])
		...
		case 3:
			//input
			fmt.Println("DEBUG: full instruction: ", mem[pc:pc+2])
		...
		case 4:
			fmt.Println("DEBUG: full instruction: ", mem[pc:pc+2])
			fmt.Println("EXECUTE: print: ", *get_val_p(&mem[pc+1], instr_str[2], &mem))
			pc += 2
		case 99:
			// halt
			//fmt.Println(mem[0])
